...
exports.createContentInstance = props => {
  ...
  if (!redLotusConfig.prettier) {
    ...
    contentText = contentText.split('\n').map(lineContent => lineContent.slice(minPrefixSpaceCount)).join('\n');
  ...
  const contentInstance = {
    ...
    appendChild(child) {
      if (this !== contentInstance) {
        throw Error(`You can only call 'appendChild' by 'contentInstance.appendChild(...), anything else` + `which will change the 'this' pointer will throw an error.`);
