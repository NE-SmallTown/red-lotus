...
const {
  RedLotusCurrent
} = require('./shared'); // TODO 这玩意没暴露出来啊？？？为啥 noop-renderer 可以 require 得到？
...
const RedLotusDomServerRenderer = ReactReconciler({
  ...
  appendChild(parent, child) {// console.log('appendChild');
  ...
  commitUpdate(instance, updatePayload, type, oldProps, newProps) {// console.log('commitUpdate', instance, updatePayload, type, oldProps, newProps);
  },

  prepareUpdate(instance, type, oldProps, newProps) {// console.log('prepareUpdate', instance, type, oldProps, newProps);
  },
...
module.exports = {
  ...
  render(element, container) {
    ...
    try {
      const fiberRoot = RedLotusDomServerRenderer.createContainer(rootContainer, ReactRootTags.BlockingRoot, false, null);
      RedLotusCurrent.currentRoot = fiberRoot;
      RedLotusDomServerRenderer.updateContainer(element, fiberRoot, null, null);
    } catch (e) {
