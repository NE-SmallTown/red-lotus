...
// TODO 起两个服务，一个 node 的去跑（支持热更），一个 webpackDevServer 的（实时预览）
...
async function start () {
  try {
    console.log(chalk.green('redlotus cli start....'));

    // 用 node 执行之前需要先编译（不然直接 node 去跑多半会报错的，很多语法 node 并不支持）
    const { output } = await build();

    console.log(chalk.green('redlotus cli bundle successfully.'));

    // 然后用 node 去编译后的入口文件即可
    console.log(chalk.green('redlotus cli execution start...'));
    const execRes = await execa('node', [ output.fileName ], { cwd: USER_ROOT });
